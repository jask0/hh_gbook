			...
			<div class="panel <?=($data['public'] == 1) ? 'panel-primary custom-panel' : 'panel-danger'; ?>">
				<div class="panel-heading <?=($data['public'] == 1) ? 'custom-heading' : ''; ?>">
					...
					<div class="pull-right">
						<?php if ($gbs['email'] && $data['email'] != '') { ?>
							<a href="mailto:<?php echo $data['email']; ?>" target="_blank" class="btn btn-xs <?=$gbs['btn_mail']?>" title="<?=$l['email']?>">
								<i class="fa fa-envelope-o"></i>
							</a>
						<?php }
						if ($gbs['homepage'] && $data['homepage'] != '') { ?>
							<a href="<?php echo $data['homepage']; ?>" target="_blank" class="btn btn-xs <?=$gbs['btn_mail']?>" title="<?=$l['homepage']?>">
				...
				<?php if($data['kommentar'] != ''){ ?>
					<div class="panel-footer <?=($data['public'] == 1) ? 'custom-footer' : ''; ?>">
						<?php echo nl2br(showBBcodes($data['kommentar'])); ?>
					</div>
				<?php } ?>
			</div>
		</div>
